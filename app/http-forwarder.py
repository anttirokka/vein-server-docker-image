...
The Vein game server binds its HTTP API to 127.0.0.1 only, inside the
container's network namespace. Docker's port publishing cannot deliver
external traffic to a loopback-only listener, so this forwarder relays
0.0.0.0:9080 -> 127.0.0.1:8080 to make it reachable at all. CORS headers
are added too, but the relay itself (not the CORS part) is why this
exists - map the host port to 9080, not 8080 directly.
"""
